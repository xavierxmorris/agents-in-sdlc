...
class TestCategoriesRoutes(unittest.TestCase):
    ...
    TEST_DATA: Dict[str, Any] = {
        ...
        "categories": [
            ...
            {"name": "Puzzle", "description": "Brain-teasing puzzle challenges"}
        ],
        "games": [
            {
    ...
    def test_get_categories_success(self) -> None:
        ...
        expected_names = {c['name'] for c in self.TEST_DATA["categories"]}
    ...
    def test_get_categories_sorted_by_name(self) -> None:
        """Test that categories are returned sorted alphabetically by name."""
        response = self.client.get(self.CATEGORIES_API_PATH)
        data = self._get_response_data(response)
